...
BTNode *BinaryTreeCreatExe(BTDataType *src){
	if (src[s_n] == '#'){
		s_n++;
		return NULL;
	}
	...
	cur->lchild = BinaryTreeCreatExe(src);
	cur->rchild = BinaryTreeCreatExe(src);
	return cur;
}
...
void BinaryTreeInOrderNonR(BTNode * root){
	...
	while (cur || !StackIsEmpty(&st)){
		...
		//第一种情况是左孩子访问完毕，第二种情况是左子树为空，无论那种，当前节点都要打印
		putchar(cur->data);
		StackPop(&st);
...
void BinaryTreePostOrderNonR(BTNode* root){
	...
	do{
		...
		while (!StackIsEmpty(&st) && tag[st.size - 1]){
			...
			//2、当cur不为空时，上面的for进，则此条件不成立
			//如果检测到当前的LT被置位（也就是情况1），那么在打印完当前节点后，再去直接检查上一个节点（父节点）是不是也要被打印了（LT置位）
			cur = StackTop(&st);
			putchar(cur->data);
			StackPop(&st);
		}
		...
		if (!StackIsEmpty(&st))//此条件只在最后一次循环跳出时生效
		{
			...
			tag[st.size - 1] = 1;
			cur = cur->rchild;//左子树访问完毕后，访问右子树
		}
	} while (!StackIsEmpty(&st));
	//由于后序遍历中根节点是最后出栈的，所以在根节点出栈前，栈不可能为空，所以以栈是否为空判定是否要跳出
	StackDestory(&st);
}
